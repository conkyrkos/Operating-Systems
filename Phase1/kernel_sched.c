...
static TCB* sched_queue_select(TCB* current)
{
	...
	{
		/* Get the head of the SCHED[highest priority] list */
...
void yield(enum SCHED_CAUSE cause)
{
	...
	if(cause==SCHED_QUANTUM && current->priority > 0){
		current->priority--;
	}else if(cause==SCHED_IO && current->priority<(PRIORITY_QUEUES-1)){
		current->priority++;
	}else if(current->curr_cause == current->last_cause && current->last_cause == SCHED_MUTEX && current->priority > 0){
		current->priority--;
	}
	
	if(sumOfYields >= YieldsToBoost){
		priority_boost();
		sumOfYields=0;	
	}
...
rlnode* node ;
int i = 0;
	for(i = PRIORITY_QUEUES-2;i>=0;i--){
		while (!is_rlist_empty(&SCHED[i]))
		{
			node = rlist_pop_front(&SCHED[i]);
			if(node->tcb->priority < PRIORITY_QUEUES-1){
				node->tcb->priority++;
			}
